...
def generate_excel_report(json_data_path, output_filepath):
    with open(json_data_path, "r", encoding="utf-8") as f:
        data = json.load(f)
        
    test_results = data.get("results", [])
    execution_mode = data.get("execution_mode", "Selenium Web Execution Engine")
    ...
    title_cell.value = "RejectionIQ - Node.js Selenium Web E2E Test Report"
    ...
    sub_cell = ws_dash["A3"]
    sub_cell.value = f"Target Environment: http://localhost:5173 | Engine: {execution_mode} | Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    kpis = [
        ("Total Web Test Cases", total_tests, "A5:B6"),
    ...
    ws_dash["A8"] = "Web Module Performance Breakdown"
    ...
    # ---------------------------------------------------------------------
    # Sheet 2: Detailed Web Test Log
    # ---------------------------------------------------------------------
    ws_log = wb.create_sheet(title="Detailed Web Test Log")
    ws_log.views.sheetView[0].showGridLines = True

    log_headers = ["Test Case ID", "Module", "Scenario Name", "Steps Executed", "Expected Result", "Actual Result", "Status", "Duration (ms)", "Browser / Engine"]
    ...
    for row_idx, r in enumerate(test_results, start=2):
        ...
        plat_cell = ws_log.cell(row=row_idx, column=9, value=execution_mode)
    ...
    col_widths = {1: 14, 2: 24, 3: 35, 4: 45, 5: 45, 6: 45, 7: 12, 8: 15, 9: 28}
    for col_idx, width in col_widths.items():
        col_letter = get_column_letter(col_idx)
        ws_log.column_dimensions[col_letter].width = width

    wb.save(output_filepath)
    print(f"[+] Excel Analysis Report saved to: {os.path.abspath(output_filepath)}")

if __name__ == "__main__":
    if len(sys.argv) >= 3:
        json_path = sys.argv[1]
        out_path = sys.argv[2]
        generate_excel_report(json_path, out_path)
